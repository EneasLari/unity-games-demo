...
function makeCard(g, i = 0) {
    ...
    const delay = `${Math.min(i, 12) * 0.05}s`;

    return `
    <article class="card" style="--delay:${delay}">
      ${badge}
      <img class="thumb" src="${g.thumb || "assets/default-thumb.png"}" alt="${escapeHtml(g.title || g.id)} thumbnail">
      <div class="cardbody">
        ...
        <div class="actions">
          ...
          <a class="btn btn-secondary" href="${directUrl}" target="_blank" rel="noopener">Direct</a>
        </div>
      </div>
    </article>
  `;
...
async function initIndex() {
    ...
    function render(filterText = "") {
        ...
        const filtered = !f
            ...
            : games.filter((g) => {
                const hay =
                    `${g.id} ${g.title} ${g.description} ${(g.tags || []).join(" ")}`
                        .toLowerCase();
        ...
        grid.innerHTML =
            filtered.map((g, i) => makeCard(g, i)).join("") ||
            `<p class="section-text">No games found.</p>`;
    }
...
(async function boot() {
    try {
        ...
        const grid = document.getElementById("grid");
        const title = document.getElementById("title");
        if (grid) grid.innerHTML = `<p class="section-text">Failed to load. Check <code>games.json</code>.</p>`;
